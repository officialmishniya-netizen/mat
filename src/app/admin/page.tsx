...
export default async function AdminDashboardPage() {
    ...
    try {
        ...
        financialStats = await db.select({
            totalLiability: sql<string>`COALESCE(SUM(amount), '0.00')`,
            totalRevenue: sql<string>`COALESCE(SUM(CASE WHEN type = 'deposit' THEN amount ELSE 0 END), '0.00')`,
            totalWithdrawn: sql<string>`COALESCE(SUM(CASE WHEN type = 'withdrawal' THEN ABS(amount) ELSE 0 END), '0.00')`,
            adRewardsToday: sql<string>`COALESCE(SUM(CASE WHEN type = 'ad_reward' AND created_at >= ${todayStr} THEN amount ELSE 0 END), '0.00')`,
        ...
        const ledgerFlowQuery = await db.select({
            ...
            deposits: sql<string>`COALESCE(SUM(CASE WHEN type = 'deposit' THEN amount ELSE 0 END), '0.00')`,
            payouts: sql<string>`COALESCE(SUM(CASE WHEN type IN ('matrix_cycle', 'matching_bonus', 'ad_reward', 'referral_bonus') THEN ABS(amount) ELSE 0 END), '0.00')`,
        ...
        try {
            ...
            console.warn("Ad views query failed");
        }
